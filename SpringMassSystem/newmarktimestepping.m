...
%     % Velocity ;
%     %%%%%%%%%%%%
%     vel(:,s+1) = AM \((gamma*dt*(ForceVec -...
%         (K*((dis(:,s)) + (dt*vel(:,s)) + (0.5*dt^2*(1-2*beta)*acc(:,s))))))...
%         + ((1-gamma)*dt*AM*acc(:,s)) + AM*vel(:,s));
%     
%     %%%%%%%%%%%%%%%
%     % Accleration ;
%     %%%%%%%%%%%%%%%
%     acc(:,s+1) = (((vel(:,s+1) - vel(:,s))/dt) - ((1-gamma)*acc(:,s)))/gamma;
%     
%     %%%%%%%%%%%%%%%%
%     % Displacement ;
%     %%%%%%%%%%%%%%%%
...
%     ForceVec = myForce(:);
    ...
    iF(s+1) = ForceVec(dofpernode*forceLoc(1)-1,s+1);
    ...
    % Velocity ;
    %%%%%%%%%%%%
%     vel(:,s+1) = AM \((dt*(ForceVec(:,s+1) -...
%         (K*((dis(:,s)) + (gamma*dt*vel(:,s))))))...
%         + AM*vel(:,s));
    
%     vel(:,s+1) = AM \((dt*(...
%         ((1-gamma)*ForceVec(:,s) + gamma*ForceVec(:,s+1)) -...
%         (K*((dis(:,s)) + (gamma*dt*vel(:,s))))))...
%         + AM*vel(:,s));
    
    gammaForceVec = (1-gamma)*ForceVec(:,s) + gamma*ForceVec(:,s+1);
    
    vel(:,s+1) = AM\...
        (dt*gammaForceVec - K*(dt*dis(:,s) + gamma*dt^2*vel(:,s))...
        + AM*vel(:,s));
    ...
    % Accleration ;
    %%%%%%%%%%%%%%%
    acc_gamma = (vel(:,s+1) - vel(:,s))/dt;
    
    %%%%%%%%%%%%%%%%
    % Displacement ;
     %%%%%%%%%%%%%%%%
    dis(:,s+1) = dis(:,s) + (dt*vel(:,s)) + 0.5*dt^2*acc_gamma;
